function [myWorksheet, newPassNames] = expandWorksheetVPsFromVPop(myWorksheet,newVPop, myMapelOptions,suffix,wsIterCounter, maxNewPerIter, myScreenTable, expandCohortSize, varyMethod, gaussianStd, maxNewPerOld, unweightedParents, selectByParent, useScoresForNew, pwExpandCutoff, myScreenFunctionName, linearExpandFlag)
...
%  linearExpandFlag:     If linearExpandFlag is true, we sample children around all VPs proportional to their PWs, and add all past-screen VPs; 
%                        if linearExpandFlag is false, we use the previous way: select high PW VPs as parent VPs, sample equally maxNewPerOld VPs to add
%                      
%                      
% RETURNS:
%  myWorksheet
...
if continueFlag   
    ...
    if linearExpandFlag % During linearExpansion phase, we select all VP as parent VPs and sample chidren number proportional to their weight. no need to go through the scoring function, just add all VPs that pass screen
        keepLowPwEdgeIndices = find(myPWs(edgeVPIndices)<0.01); 
        edgeVPIDs = originalVPIDs(edgeVPIndices(keepLowPwEdgeIndices));
        ...
        if ~isempty(newVPop.LinearProblemMatrices)
            ...
            if (~isempty(newVPop.LinearProblemMatricesSubgroupSumWeights))
                ...
                lowsubpop = find(sum(uniqueSubpop,2)'<= 5); 
            end
            ...
            subpopVPIDs = originalVPIDs(subpopvpIndices);
            subpopVPIDs = setdiff(subpopVPIDs,edgeVPIDs);
            subpopvpIndices = nan(1, length(subpopVPIDs));
            for vpCounter = 1 : length(subpopVPIDs)
                subpopvpIndices(vpCounter) = find(ismember(originalVPIDs,subpopVPIDs{vpCounter}));
            end 
        else
            subpopVPIDs = {};
            subpopvpIndices = [];
        ...
        % sample children VPs proportionally to the parentVP weight
        highVPIDsMono = setdiff(highVPIDs,union(edgeVPIDs,subpopVPIDs)); % can also consider taking out subpop VPs
        AddParentVPIDs = highVPIDsMono; % union(highVPIDs,edgeVPIDs);
        allVPIDs = getVPIDs(myWorksheet);
        AddParentVPIndices = [];
        for i=1:length(AddParentVPIDs)
            ind = find(ismember(allVPIDs,AddParentVPIDs{i}));
            AddParentVPIndices = [AddParentVPIndices, ind];
        end

        AddParentVPpws = myPWs(AddParentVPIndices)./sum(myPWs(AddParentVPIndices));
        if length(edgeVPIDs)>0
            expandedgeVPCohortSize = max(length(edgeVPIDs)*maxNewPerOld,expandCohortSize*0.1);  
        else
            expandedgeVPCohortSize = 0;
        end
         % allocate 10% to sample around low subpopulation VPs
        % since they are all low weight, maybe just sample equally here
        if length(subpopVPIDs)>0
            expandlowsubpopVPCohortSize = max(length(subpopVPIDs)*maxNewPerOld,expandCohortSize*0.1);  
        else
            expandlowsubpopVPCohortSize = 0;
        end
         %
        expandhighVPCohortSize = expandCohortSize-expandedgeVPCohortSize-expandlowsubpopVPCohortSize;
        CountUniqueParentVPs = round(AddParentVPpws.*expandhighVPCohortSize);
        ...
        disp('---')  
        disp(['Adding ',num2str(length(AddParentVPIDs)),' VPs as expansion seeds from prevalence weight considerations in ',mfilename,'.'])
        disp(['Adding ',num2str(length(edgeVPIDs)),' VPs as expansion seeds from phenotype range considerations in ',mfilename,'.'])
        disp(['Adding ',num2str(length(subpopVPIDs)),' VPs as expansion seeds from low subpopulation considerations in ',mfilename,'.'])
        
        % sample children VPs proportionally to the parentVP weight
        jitteredWorksheet = myWorksheet;
        for addCounter = 1: length(AddParentVPIDs)
            if CountUniqueParentVPs(addCounter)>0
                myVaryAxesOptions.newPerOld = CountUniqueParentVPs(addCounter);
               % CountUniqueParentVPs(addCounter)
                myVaryAxesOptions.baseVPIDs = AddParentVPIDs(addCounter);
               % AddParentVPIDs(addCounter)
        ...
        for addCounter = 1: length(edgeVPIDs)
                myVaryAxesOptionsEdge.newPerOld = round(expandedgeVPCohortSize/length(edgeVPIDs)); %maxNewPerOld;
                myVaryAxesOptionsEdge.baseVPIDs = edgeVPIDs(addCounter);
                jitteredWorksheet = addVariedVPs(jitteredWorksheet, myVaryAxesOptionsEdge);
        end

        % low subpopulation VPs
        myVaryAxesOptionsSubpop = myVaryAxesOptions;
        myVaryAxesOptionsSubpop.gaussianStd = 0.05;
        myVaryAxesOptionsSubpop.varyMethod = 'gaussian';
        for addCounter = 1: length(subpopVPIDs)
                myVaryAxesOptionsSubpop.newPerOld = round(expandlowsubpopVPCohortSize/length(subpopVPIDs)); %maxNewPerOld;
                myVaryAxesOptionsSubpop.baseVPIDs = subpopVPIDs(addCounter);
                jitteredWorksheet = addVariedVPs(jitteredWorksheet, myVaryAxesOptionsSubpop);
        end
        ...
        myWorksheet = jitteredWorksheet;        
        curVPIDs = getVPIDs(jitteredWorksheet);
        newIndices = (find(~ismember(curVPIDs,originalVPIDs)));
        newVPIDs = curVPIDs(newIndices); 
        newPassNames = newVPIDs;
    else
        ...
        % end
        % Zero out scores if we are going to ignore them when selecting
        % which children to keep.
        if ~useScoresForNew
            newVPScores = newVPScores*0;
        end
        if unweightedParents
